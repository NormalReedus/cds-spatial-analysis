...
```{r}
suburbs <- st_read("data/bydel.shp", options = "ENCODING=WINDOWS-1252") # æøå
...
## Flats through time
```{r view-flats}
tmap_options(limits = c(facets.view = 6)) # we want to view 6 years (knitting might not do more than 4 per default)
tmap_mode(mode = "view" )
tm_shape(wc_spatial) +
  tm_facets(by = "year")+
  tm_polygons("flats",
