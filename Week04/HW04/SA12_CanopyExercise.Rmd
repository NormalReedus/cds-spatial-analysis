...
# avg_canopy does not consist of numbers that tmap wants to plot, so we convert it first
neighborhoods <- mutate(neighborhoods, avg_canopy = as.numeric(avg_canopy))
...
map2 <- tm_shape(neighborhoods) +
  tm_polygons(col = 'avg_canopy', palette = 'Greens', 
        title = "Average tree canopy (%)")

map3 <- tm_shape(neighborhoods) +
  tm_polygons(col = 'tree_density', palette = 'Greens', 
        title = "Trees per sq. KM")

tmap_arrange(map1 + map2, map1, map1 + map3, asp = NA)
```
